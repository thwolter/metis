...
from datasifter.interfaces import RetrievalProvider as RetrievalProviderInterface
...
class RetrievalProvider(RetrievalProviderInterface):
    def __init__(self, session: AsyncSession):
        self._session = session

    async def _load_header_weights(self, doc_type: str) -> list[tuple[str, bool, float]]:
        stmt = select(HeaderWeight.pattern, HeaderWeight.is_regex, HeaderWeight.weight).where(
            HeaderWeight.class_name == doc_type
        )
    ...
    async def retrieve(
        ...
        *,
        ...
        tenant_id = job.tenant_id or request.tenant_id
        digest = job.document_digest or request.digest
        collection_name = job.collection_name or request.collection_name
        if tenant_id is None or digest is None or collection_name is None:
            msg = 'tenant_id, digest, and collection_name are required for retrieval'
        ...
        header_weights = await self._load_header_weights(request.doc_type)
